...
from models.viewformer import BaseImageClassifier, ViewFormer
from datasets.data import SH17_subclass_MultiView
...
# --- 1.2 load pretrained model weights
# e.g., viewformer
method_name = sys.argv[5]
# e.g., `SH17_sub-V20-L4H8D512-MR2-Alex-1`
exp_name = sys.argv[6]
...
# --- 1.3 define multi-view image data set
# e.g. normal, perturbed
version = sys.argv[7]
total_num_views = 20
mv_test_set = SH17_subclass_MultiView(root_dir='../data/shrec17', label_file=label_file, 
...
# --- 1.4 load multi-view image dataset
# e.g., 132
...
# --- 1.5 forward pass to get shape `subcategory distribution`
with torch.no_grad():
    pred_logits = []
    pred_subclass_ids = []
    for data in mv_test_loader:
        ...
        subclass_ids = logits.argmax(axis=1)

        pred_logits.extend(logits)
        pred_subclass_ids.extend(subclass_ids)

pred_logits = np.array(pred_logits)
pred_subclass_ids = np.array(pred_subclass_ids)

print('pred_logits.shape:', pred_logits.shape)
print('pred_subclass_ids.shape:', pred_subclass_ids.shape)
...
# NOTE 生成 test.csv 和 fp_test_sub.txt
with open(f'{split}_sub.csv', 'w') as fout1, open(f'fp_{split}_{version}_sub.txt', 'w') as fout2:
    fout1.write('id,subSynsetId,split\n')
    for fp in filepaths:
        filename = basename(fp)
        shape_name = filename[:6]
        shape_names.append(shape_name)

        subclass_name = mv_test_set.shape2subclass[shape_name]
        subclass_id = mv_test_set.subclassnames.index(subclass_name)

        fout1.write(f'{shape_name},{subclass_id},{split}\n')
        fout2.write(f'{fp}\n')

shape_names = np.array(shape_names)
num_objects = len(pred_logits)

savedir = join('evaluator', method_name, f'{split}_{version}_sub')
if not exists(savedir):
    os.mkdir(savedir)

for idx in range(num_objects):
    filename = join(savedir, shape_names[idx])
    with open(filename, 'w') as fout:   # viewformer/test_normal_sub/000009
        scores_column = pred_logits[:, pred_subclass_ids[idx]].copy()
        scores_column[idx] = float('inf')
        
        # pick up sample ids in the same class
        ids = [i for i, class_id in enumerate(pred_subclass_ids) if class_id == pred_subclass_ids[idx]]
