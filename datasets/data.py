...
class SONN_MultiView(torch.utils.data.Dataset):
    def __init__(self, root_dir, scale_aug=False, rot_aug=False, test_mode=False, 
                 num_views=20, total_num_views=20, num_classes=15):
        assert num_classes == 15, '`num_classes` should be equivalent to `15`'
        ...
        self.num_views = num_views
        set_ = self.root_dir.split('/')[-1]  # --> train
        parent_dir = self.root_dir.rsplit('/',2)[0]  # --> data/sonn_hardest
        self.filepaths = []
        for i in range(len(self.classnames)):
            # sorted 这个方法很巧妙
            all_files = sorted(glob.glob(parent_dir+'/'+self.classnames[i]+'/'+set_+'/*.png'))
...
class SONN_SingleView(torch.utils.data.Dataset):
    def __init__(self, root_dir, scale_aug=False, rot_aug=False, test_mode=False, num_classes=15):
        assert num_classes == 15, '`num_classes` should be equivalent to `15`'
        self.classnames=['bag', 'bin', 'box', 'cabinet', 'chair', 'desk', 'display',
                        'door', 'shelf', 'table', 'bed', 'pillow', 'sink', 'sofa', 'toilet']
...
class SH17_subclass_MultiView(torch.utils.data.Dataset):
    def __init__(self, root_dir='data/shrec17', label_file='train.csv',  
                 version='normal', num_views=20, total_num_views=20, num_classes=203):
        assert num_classes in [203,], '`num_classes` should be chosen in this list: [203,]'

        self.shape2subclass = dict()
        with open(os.path.join(root_dir, label_file)) as fin:
            lines = fin.readlines()
            for i, line in enumerate(lines):
                if i == 0:
                    continue    # skip csv header line
                shape_name, _, subclass_name = line.strip().split(',')
                if shape_name not in self.shape2subclass.keys():
                    self.shape2subclass[shape_name] = subclass_name
        ...
        self.subclassnames = self.get_subclasses()
        self.transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                    std=[0.229, 0.224, 0.225])
    ...
    def get_subclasses(self):
        subclassnames = list()
        with open(os.path.join(self.root_dir, 'train.csv')) as fin:
            lines = fin.readlines()
            for i, line in enumerate(lines):
                if i == 0:
                    continue    # skip csv header line
                _, _, subclass_name = line.strip().split(',')
                if subclass_name not in subclassnames:
                    subclassnames.append(subclass_name)
        # it is necessary to sort `subclassnames`, ensuring `subclassnames` in order in train/test/val 
        return sorted(subclassnames)
    ...
    def __getitem__(self, idx):
        ...
        shape_name = img_name[:6]
        # e.g. '04256520'
        subclass_name = self.shape2subclass[shape_name]
        subclass_id = self.subclassnames.index(subclass_name)
        ...
        return (subclass_id, torch.stack(imgs), self.filepaths[idx*self.num_views:(idx+1)*self.num_views])


class SH17_subclass_SingleView(torch.utils.data.Dataset):
    def __init__(self, root_dir='data/shrec17', label_file='train.csv', version='normal', num_classes=203):
        assert num_classes in [203,], '`num_classes` should be chosen in this list: [203,]'
