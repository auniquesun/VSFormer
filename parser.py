...
parser = argparse.ArgumentParser(description='VSFormer for Multi-view 3D Shape Analysis')

# --------- initialize project
parser.add_argument('--proj_name', type=str, default='VSFormer', metavar='N',
...
parser.add_argument('--vsformer_weights', type=str, default='mv_model_best.pth', metavar='N',
                    help='saved vsformer name')
...
parser.add_argument('--num_img_proj_layers', type=int, default=1, help='Number of layers of image feature extractor')
parser.add_argument('--num_layers', type=int, default=4, metavar='N', help='Number of self attention blocks')
...
parser.add_argument('--trial_id', type=int, default=1, help='trial id of RGBD-dataset_eval')
parser.add_argument('--num_channels', type=int, default=512,
                    help='feature dimension of input views')
parser.add_argument('--num_patch_channels', type=int, default=256,
                    help='feature dimension of patches in input views')
parser.add_argument('--num_heads', type=int, default=4, metavar='N', 
                    help='Number of heads in the view attention layer')
parser.add_argument('--num_patch_heads', type=int, default=4, metavar='N', 
                    help='Number of heads in the patch attention layer')
...
parser.add_argument('--clshead_layers', type=int, default=2, help='the number of layers in CLSHead')
parser.add_argument('--use_cls_token', action='store_true', help='flag to use class token in Transformer input')
parser.add_argument('--use_max_pool', action='store_true', help='flag to use max pooling in Transformer output')
parser.add_argument('--use_mean_pool', action='store_true', help='flag to use mean pooling in Transformer output')
parser.add_argument('--use_pos_embs', action='store_true', help='flag to use position encodings in Transformer input')
parser.add_argument('--use_view_patches', action='store_true', help='flag to use view patches attention')
